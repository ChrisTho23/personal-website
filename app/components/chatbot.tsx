...
const ChatBot = () => {
    ...
    return (
        <div className="fixed bottom-5 right-5 z-50">
            {isOpen ? (
                <div className="w-96 h-[600px] p-4 bg-zinc-800 text-white rounded-lg shadow-lg flex flex-col">
                    <div className="flex justify-between items-center mb-2">
                        <h2 className="text-xl font-bold">ChrisGPT</h2>
                        <button onClick={handleCloseChat} className="text-zinc-400 text-sm p-1">
                            ×
                        </button>
                    </div>
                    <div className="flex-1 overflow-y-auto p-2 border border-zinc-700 rounded mt-2 mb-4">
                        {messages.map((message, index) => (
                            <div
                                key={index}
                                className={`mb-2 p-2 rounded-lg max-w-xs ${message.role === 'user' ? 'bg-blue-500 text-white self-end mr-4' : 'bg-zinc-700 text-white self-start ml-4 text-right'}`}
                            >
                                {message.content}
                            </div>
                        ))}
                    ...
                    <form onSubmit={handleSubmit} className="flex mt-2 items-center">
                        <button
                            ...
                            className="bg-gray-300 text-black font-bold h-12 w-12 rounded-full flex items-center justify-center"
                        >
                        ...
                        <textarea
                            ...
                            className="flex-1 h-12 p-2 border border-zinc-700 rounded-full resize-none ml-2 bg-zinc-700 text-white placeholder-zinc-400 flex items-center"
                            placeholder="Type your message..."
                            rows="1"
                            style={{ display: 'flex', alignItems: 'center' }}
                        />
                        <button type="submit" className="ml-2 bg-gray-300 text-black font-bold h-12 px-4 rounded-full flex items-center justify-center">
                            Send
                        </button>
                    </form>
                </div>
                ...
                <button
                    className="h-14 w-24 bg-zinc-800 rounded-full text-white text-lg p-3 flex items-center justify-center shadow-lg"
